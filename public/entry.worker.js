...
var require_gun = __commonJS({
  "node_modules/gun/gun.js"(exports, module) {
    (function() {
      ...
      USE(function(module2) {
        ...
        var parse = JSON.parseAsync || function(t, cb, r) {
          var u2, d = +new Date();
          try {
            cb(u2, JSON.parse(t, r), json2.sucks(+new Date() - d));
          } catch (e) {
            cb(e);
          }
        };
        var json2 = JSON.stringifyAsync || function(v, cb, r, s) {
          var u2, d = +new Date();
          try {
            cb(u2, JSON.stringify(v, r, s), json2.sucks(+new Date() - d));
          } catch (e) {
            cb(e);
          }
        };
        json2.sucks = function(d) {
          if (d > 99) {
        ...
        function Mesh(root) {
          ...
          (function() {
            ...
            mesh.hash = function(msg, peer) {
              var h, s, t;
              var S = +new Date();
              json2(msg.put, function hash(err, text) {
      ...
      USE(function(module2) {
        ...
        Gun.on("create", function lg(root) {
          ...
          root.on("put", function(msg) {
            this.to.next(msg);
            var put = msg.put, soul = put["#"], key = put["."], id = msg["#"], tmp;
    ...
    (function() {
      ...
      Type.list.map = Type.list.map || function(l, c, _2) {
        DEP("list.map");
        return obj_map(l, c, _2);
      };
      Type.list.index = 1;
      ...
      (function() {
        ...
        Type.obj.map = map = Type.obj.map || function(l, c, _2) {
          DEP("obj.map");
          ...
          if (list_is(l) || ll) {
            ...
            for (; i < x; i++) {
              ...
              if (f) {
                r = lle ? c.call(_2, l[ll[i]], ll[i], t) : c.call(_2, l[i], ii, t);
                if (r !== u3) {
                  return r;
                }
              } else {
            ...
            for (i in l) {
              if (f) {
                if (obj_has(l, i)) {
                  r = _2 ? c.call(_2, l[i], i, t) : c(l[i], i, t);
                  if (r !== u3) {
                    return r;
                  }
                }
...
// packages/no-scope/debug.ts
var collapse = console.groupCollapsed.bind(console.trace);
function debug({ off, devOnly = true }) {
  let isProd = false;
  return {
    log(...args) {
      (devOnly && !isProd || !devOnly) && console.log(source_default.green(...args));
    },
    warn(...args) {
      (devOnly && !isProd || !devOnly) && console.log(source_default.yellow.italic(...args));
    },
    error(...args) {
      (devOnly && !isProd || !devOnly) && console.log(source_default.red.bold(...args));
      (devOnly && !isProd || !devOnly) && collapse(source_default.italic(...args));
    },
...
async function handleFetch(event) {
  ...
  if (isLoaderRequest(event.request)) {
    try {
      ...
      _.error("Serving data from network failed, falling back to cache", url.pathname + url.search);
  ...
  if (isDocumentGetRequest(event.request)) {
    try {
      ...
      _.error("Serving document from network failed, falling back to cache", url.pathname);
      let response = await caches.match(event.request);
      if (response) {
        return response;
      }
