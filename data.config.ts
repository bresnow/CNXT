export const data = {
    pages: {
        root: {
            meta: {
                title: "Remix.GUN | Boilerplate",
                description: `P2P content management system using Remix.run and GunDB. Styles with Tailwind.css`,
                author: "Bresnow",
                keywords: "floating, mammoth, FLTNGMMTH, Bresnow, development, server, Remix.run, GunDB, gun",
                "og:image": `https://${process.env.DOMAIN}/github/rmix-gun.png`,
                "og:image:width": "512",
                "og:image:height": "512",
                "og:description": "P2P content management system using Remix.run and GunDB. Styles with Tailwind.css"
            },

        },
        index: {
            pageTitle: "Remix.GUN | Boilerplate ",
            src: "/github/rmix-gun.png",
            title: 'Click Here To Edit',
            pageText: "Build the data object by entering the key and value. The data object will be saved to the graph at path 'posts.test'. The object will be fetched from the graph and displayed on the page. You can access the data object from any other connected peer.",
        },
    },
}




