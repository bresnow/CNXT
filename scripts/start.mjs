...
if (args.length > 0) {
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg.startsWith('--build' || '-b')) {
            await $`cross-env NODE_ENV=production remix build`.pipe($`cross-env-shell NODE_ENV=production PUB=$PUB PRIV=$PRIV EPUB=$EPUB EPRIV=$EPRIV DOMAIN=$DOMAIN CLIENT_PORT=$CLIENT_PORT PEER_DOMAIN=$PEER_DOMAIN node ./build/index.js"`)
        }
        else {
