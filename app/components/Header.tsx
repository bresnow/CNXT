...
const Header = (props: Props) => {
  ...
  return (
    <div>
      <nav
        className={`bg-gray-800 ${props.withShadow ? " shadow" : ""}${
          props.isFat ? " py-4" : ""
        } `}
      >
        <div className="max-w-7xl mx-auto px-8">
          <div className="flex items-center justify-between h-18 -mp-3">
            <div
              className={`${
                props.alignRight ? "w-full justify-between" : ""
              } flex items-center`}
            >
              <div className="p-3">{props.logo}</div>
                  ...
                  {props.links.map((link) => {
                    return (
                      <Link key={link.label} to={link.link || "#"}>
                        <a
                          key={link.label}
                          className={` hover:text-gray-800 dark:hover:text-white px-3 py-2 rounded-md ${
