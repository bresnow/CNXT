...
export default function NameSpaceRoute() {
  ...
  let deferred = useFetcherAsync(`/api/v1/gun/g?`, {
    params: { path: `pages.tags.namespaces.${namespace}` },
  });
...
export function SuspendedProfile({ response }: { response: () => any }) {
  let { title, description, profilePic, socials } = response();
  console.log({ title, description, profilePic, socials }, 'kjsdbkjsbdakj');
